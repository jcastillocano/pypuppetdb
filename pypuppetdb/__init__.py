...
def connect(api_version=3, host='localhost', port=8080, urlproxy='',
        username=None, password=None,
        ssl_verify=False, ssl_key=None, ssl_cert=None, timeout=10):
    ...
    """
    if api_version == 3:
        return v3.API(host=host, port=port, urlproxy=urlproxy, username=username,
            password=password, timeout=timeout, ssl_verify=ssl_verify,
            ssl_key=ssl_key, ssl_cert=ssl_cert)
    if api_version == 2:
        return v2.API(host=host, port=port, urlproxy=urlproxy, username=username,
            password=password, timeout=timeout, ssl_verify=ssl_verify,
            ssl_key=ssl_key, ssl_cert=ssl_cert)
    else:
